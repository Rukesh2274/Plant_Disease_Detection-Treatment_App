    ...
    Scaffold(
        topBar = {
            if (routeName !in listOf("login", "signup","CommunityPage")) {
                TopAppBar(title = { Text(text = "Crop Disease",
                        ...
                        tabItems.forEachIndexed { index, item ->
                            NavigationBarItem(
                                ...
                                icon = {
                                    when (item) {
                                        "Home" -> Icon(
                                            if (parentRouteName == item) Icons.Filled.Home else
                                                Icons.Outlined.Home,
            ...
            NavHost(navController = navController, startDestination = if (sessionViewModel.isLoggedIn()) "Home" else "login") {
                ...
                navigation(startDestination = "CommunityPage", route = "Community") {
                    composable(
                        ...
                        ImageTextList(navController = navController,
                            viewModel = FirestoreImageTextViewModel())
                    }
                    composable("{documentId}",
                        arguments = listOf(navArgument("documentId") { type = NavType.StringType })) { navBackStackEntry ->
                ...
                composable(route = "edit") {
                    FirestoreImageTextInput(viewModel = FirestoreImageTextViewModel(), navController)
                }
